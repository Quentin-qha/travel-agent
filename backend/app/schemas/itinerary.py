...
class DateRange(BaseModel):
    ...
    to_date: date = Field(alias="to", description="Departure date (YYYY-MM-DD), included in the plan.")
...
class ItineraryRequest(BaseModel):
    ...
    traveler_count: int = Field(alias="travelerCount", gt=0, description="Number of travelers, must be > 0.")
...
class Activity(BaseModel):
    ...
    image_url: str | None = Field(
        default=None, description="Google Places photo of the place, filled in afterwards if found."
    )
...
class ItineraryResponse(BaseModel):
    ...
    image_url: str | None = Field(default=None, description="Cover photo for the destination.")
